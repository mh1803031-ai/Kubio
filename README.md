 Kubio - AI Powered Web Project

**Kubio** adalah platform portofolio pribadi saya dalam bentuk digital

## 🚀 Demo
Lihat website yang sudah dideploy di sini: [https://kubio.netlify.app](https://kubio.netlify.app)

## ✨ Fitur Utama
- **AI-Driven Design:** Pembuatan layout otomatis berdasarkan input pengguna.
- **Fully Responsive:** Tampilan optimal di perangkat mobile, tablet, maupun desktop.
- **Fast Performance:** Dioptimalkan untuk kecepatan akses tinggi menggunakan Netlify Edge.
- **Customizable Blocks:** Komponen UI yang mudah dimodifikasi sesuai kebutuhan.

## 🛠️ Teknologi yang Digunakan
- **Frontend:** HTML5 & CSS3
- **Styling:** Tailwind CSS
- **Hosting/Deployment:** Netlify
- **Icons & Assets:** FontAwesome / Unsplash

## 📦 Cara Menjalankan Proyek Secara Lokal

1. Clone repositori ini:
   ```bash
   git clone [https://github.com/username-kamu/kubio.git](https://github.com/username-kamu/kubio.git)

## Lisensi
[MIT License](LICENSE)

